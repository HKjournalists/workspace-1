...
public class MyClient2 {
	...
	private final String proxyInUse = ResourceBundle.getBundle("file").getString("useproxy");
	private final String readFromCache = ResourceBundle.getBundle("file").getString("readfromcache");
	private final String HTML_CACHE_PATH = ResourceBundle.getBundle("file").getString("htmlCachePath");
	...
	public MyClient2(String proxyAddress, int port) {
		//hc = new HttpClient();
		...
		if (proxyList.size() > 0) {
			...
			for (String value : proxyList) {
				...
				if (m.find()) {
					...
					findProxy = true;
					if (postOrGet.equalsIgnoreCase("post")) break;
	...
	//缓存 post&get 
	public RequestResult getHtml(String url, String charset) {
	...
	public RequestResult getMethodHtml(String url, String charset) {
		...
		RequestResult result = new RequestResult();
		result.setUrl(url);
		if ("true".equalsIgnoreCase(proxyInUse)) useProxy(url, "get");
		String page = "";
		GetMethod get = null;
		if (charset.equals("")) {
			charset = "GBK";
	...
	public RequestResult postHtml(String url, String body, String charset) {
		System.out.println("POST[" + url + ']');
		String page = "";
		RequestResult result = new RequestResult();
		result.setUrl(url);

		if ("true".equalsIgnoreCase(proxyInUse)) useProxy(url, "post");
	...
	public static void main(String[] args) throws Exception {
		...
		String html = client.getHtml("http://tianjin.ctsgz.cn/daohang/shengnei_index.do", "utf-8").getHtml();
