...
public class StringUtil {
	...
	public static String fillString(String str, Vector<String> args) {
	...
	public static void main(String[] args) {
		...
		String s = "第5天  第五天 d5 .:!<海南双飞5日>尊贵欢乐之旅，      海口进出";
		System.out.println(normalizeDayTitle(s));
	...
	public static String getTableHtmlWithoutStyle(Node tableNode, NekoHtmlParser parser) {
		StringBuilder result = new StringBuilder();
		result.append("<table>");
		NodeList rowList = parser.selectNodes("TBODY/TR|TR", tableNode);
		if (rowList == null) {
		...
		for(int i = 0; i < rowList.getLength(); i++) {
			...
			result.append("<tr>");
			NodeList columnList = parser.selectNodes("TD|TH" , row);
			for(int j = 0; j < columnList.getLength(); j++) {
				Node column = columnList.item(j);
				result.append("<td");
				NamedNodeMap attribs = column.getAttributes();
				for(int k = 0; k < attribs.getLength(); k++) {
					String attribsName = attribs.item(k).getNodeName();
					if (attribsName == null) continue;
					if (attribsName.toUpperCase().equals("COLSPAN") || attribsName.toUpperCase().equals("ROWSPAN")) {
						result.append(" ").append(attribsName).append("=\"").append(attribs.item(k).getNodeValue()).append("\" ");
					}
				}
				result.append(" >");
				result.append(StringUtil.trim(column.getTextContent()));
				result.append("</td>");
			}
			result.append("</tr>");
		}		
